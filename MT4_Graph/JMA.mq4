...
//----
  // if (firstTime || (RunType == Repaint) || (RunType == NoRepaint2Array))
   {
      vv=0;
      v1=0;
      v2=0;
   ...
   for( shift=limit;shift>=0;shift-- )
     {
      switch ( PriceType )
      {
         ...
         case PRICE_CLOSE:
            series = Close[shift];
         break;
         case PRICE_MEDIAN: // (high + low)/2
            series = ( High[shift] + Low[shift] ) / 2.0;
         break;
         case PRICE_TYPICAL: // (high + low + close)/3
            series = ( High[shift] + Low[shift] + Close[shift] ) / 3.0;
         break;
         case PRICE_WEIGHTED: // (high + low + close + close)/4
            series = ( High[shift] + Low[shift] + 2 * Close[shift] ) / 4.0;
         break;
                  ...
                  if (fE8<=29)
                    {
                     ...
                     v6=29;
                  fA8=(series - buffer[curIndex - v5]) * (1 - f68)/fE0 + (series - buffer[curIndex - v6]) * f68/fE8;
                 }
              }
            else
              {
